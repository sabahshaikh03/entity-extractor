...
class FileAnalysisService:
    def __init__(self, logger, global_constants):
        ...
        self.azure_vision_ai_connector = AzureVisionConnector(self.global_constants)
        ...
        self.share_point_connector = SharePointConnector(self.global_constants)
        self.sharepoint_client = self.share_point_connector.get_client()

        self.blob_storage_util = BlobStorage(
            self.blob_storage_container_client, self.logger, self.global_constants
        )
        self.sharepoint_util = SharePoint(self.sharepoint_client, self.global_constants)
        ...
        self.container_name = self.kv_client.get_secret(
            self.global_constants.keys_of_key_vault_secrets.blob_storage_container_name
        )

        self.blob_storage_base_uri = self.kv_client.get_secret(
            self.global_constants.keys_of_key_vault_secrets.blob_storage_base_uri
        )
    ...
    def search_keywords_in_extracted_text(
        ...
        for search_key in keywords:
            for block in extracted_text_data[
                self.constants.vision_ai_api_response_parameters.blocks
            ]:
                for line in block[
                    self.constants.vision_ai_api_response_parameters.lines
                ]:
                    for word in line[
                        self.constants.vision_ai_api_response_parameters.words
                    ]:
                        # Search for pattern
                        ...
                        if (
                            ...
                            page_analysis[
                                self.constants.page_analysis_parameters.matched_keywords
                            ].append(
                                {
                                    self.constants.page_analysis_parameters.keyword: search_key,
                                    self.constants.page_analysis_parameters.word: word[
                                        "text"
                                    ],
                                    self.constants.page_analysis_parameters.confidence: confidence,
                                    self.constants.page_analysis_parameters.coordinates: bounding_polygon_json,
                                }
                            )

        page_analysis[
            self.constants.page_analysis_parameters.unique_matched_keywords_count
        ] = len(keywords_matched_on_page)
        page_analysis[
            self.constants.page_analysis_parameters.matched_keywords_count
        ] = len(words_matched_on_page)
    ...
    def extract_text_from_image_using_azure_vision_api(
        ...
        vision_wait_duration = self.global_constants.vision_wait_duration
        vision_max_duration = self.global_constants.queue_visiblity_time
        ...
        while True:
            try:
                ...
                ocr_processing_complete_time = (
                    time.time() - ocr_processing_start_time
                ) * 1000
                self.logger.info(
                    f"Thread : {thread_id} :: {file_name} : Page No - {page_no} : OCR response received in {ocr_processing_complete_time} milliseconds"
                )
                break
                ...
                if (
                    ex.status_code
                    == self.global_constants.api_status_codes.rate_limit_exceeded
                ):
                    self.logger.info(
                        f"Thread : {thread_id} :: {file_name} : Page No - {page_no} : API call limit exceeded. Retrying after {vision_wait_duration} seconds."
                    )
                    if (
                        time.time() - start_time + vision_wait_duration
                    ) >= vision_max_duration * 0.8:
                        raise MaxProcessingTimeExceededException(
                            "Maximum duration exceeded."
                        )  # Raise exception if maximum duration is exceeded
                        break

                    time.sleep(vision_wait_duration)
                    temp_vision_wait_duration = vision_wait_duration * 2
                    vision_wait_duration = (
                        temp_vision_wait_duration * 0.8
                        + random.random() * (temp_vision_wait_duration * 0.2)
                    )  # Calculate new duration with jitter
        return result
    ...
    def get_keywords_from_global_keywords(self, thread_id):
        try:
            blob_client = self.blob_storage_container_client.get_blob_client(
                f"{self.constants.keyword_analysis_results_folder}/{self.constants.file_names.global_keywords}"
            )
    ...
    def update_progress_status(self, status, file_path, content="", thread_id=-1):
        ...
        for file_name in self.constants.status_files.values():
            try:
        ...
        self.blob_storage_util.upload_file_to_blob_storage(
            type=self.constants.file_types.lock,
            file_data=content,
            file_path=f"{file_path}/{self.constants.status_files[status]}",
        )
    ...
    def document_to_pdf(
        ...
        file_name,
        queue_item,
        base64_encoded_file_uri,
    ):
        max_processing_time = int(self.global_constants.queue_visiblity_time) * 0.9
        start_time = time.time()
        while True:
            try:
                if (time.time() - start_time) > max_processing_time:
                    ...
                    self.logger.warn(
                        f"Thread : {thread_id} :: {file_name} : Conversion time reached for file {base64_encoded_file_uri} - increasing visibility timeout to {max_processing_time}.",
                    )
                ...
                subprocess.run(
                    ...
                    check=True,
                    timeout=self.constants.conversion_timeout,
                )
                return queue_item
            except subprocess.TimeoutExpired as e:
                self.logger.error(
                    f"Thread : {thread_id} :: {file_name} : File is not converted from Document to PDF due to timeout: {str(e)}"
                )
                raise Doc2PDFTimeoutError("Doc2PDFTimeoutExpired")
            except subprocess.CalledProcessError as e:
                delta = time.time() - start_time
                if delta > (2 * 60 * 60):  # 120 minutes
                    raise Doc2PDFConversionError(
                        f"Thread : {thread_id} :: {file_name}: Failed trying to convert using conversion library,Conversion exceeded 120 minutes"
                    )
                else:
                    sleep_time = random.uniform(2, 10)
                    self.logger.warning(
                        f"Thread : {thread_id} :: {file_name}: Conversion failed, retrying in {sleep_time} seconds..."
                    )
                    time.sleep(
                        sleep_time
                    )  # Retry after a random time between 8 and 10 seconds

    def get_status_of_file_if_already_processed(self, directory_path):
        status_files = {
            self.constants.statuses.processing: f"{directory_path}/{self.constants.file_names.processing_lock}",
            self.constants.statuses.finished: f"{directory_path}/{self.constants.file_names.finished_lock}",
            self.constants.statuses.failed: f"{directory_path}/{self.constants.file_names.failed_lock}",
            self.constants.statuses.queued: f"{directory_path}/{self.constants.file_names.queued_lock}",
        }
    ...
    def get_file_path_from_blob_uri(self, parsed_url):
        ...
        if query:
            file_path += self.constants.query_separator + query
        if fragment:
            file_path += self.constants.fragment_separator + fragment

        return file_path

    def get_input_file_from_input_source(
        self,
        source,
        file_uri,
        file_name,
        ...
        if source == self.constants.input_file_sources.sharepoint:
        ...
        # Read file from Blob storage
        elif source == self.constants.input_file_sources.blob:
            ...
            file_byte_stream = (
                self.blob_storage_util.get_file_as_bytes_from_blob_storage(file_path)
            )
            self.logger.info(
        ...
        if (
            file_type not in self.global_constants.doc_mime_types
            and file_type != self.global_constants.pdf_file_type
        ):
            self.logger.warning(
        ...
        if file_type in self.global_constants.doc_mime_types:

            TempPdfComplete = False
            TempDocComplete = False
            try:
                self.logger.info(
                    f"Thread : {thread_id} :: {file_name} : Document file received for analysis"
                )

                # Encode file name in base64
                filename_without_extension = os.path.splitext(file_name)[0]
                base64_file_name = self.encoder.encode_data(filename_without_extension)

                # Create a Temporary PDF File with base64 encoded name
                with NamedTemporaryFile(
                    delete=False,
                    suffix=self.global_constants.pdf_extension,
                    prefix=base64_file_name,
                ) as temp_pdf_file:
                    with open(
                        temp_pdf_file.name.replace(
                            self.global_constants.pdf_extension,
                    ...
                    self.logger.info(
                        f"Thread : {thread_id} :: {file_name} : Document file converting to PDF format from {temp_doc_file.name} to {temp_pdf_file.name}"
                    )

                    # Convert DOC/DOCX to PDF
                    queue_item = self.document_to_pdf(
                        temp_doc_file.name,
                        os.path.dirname(temp_pdf_file.name),
                        thread_id,
                        file_name,
                        queue_item,
                        base64_encoded_file_uri,
                    ...
                    self.logger.info(
                        f"Thread : {thread_id} :: {file_name} : Document file converted to PDF format"
                    )
            ...
            except Doc2PDFTimeoutError as e:
                raise e
        ...
        return pdf_document, pdf_document.page_count, queue_item
    ...
    def extract_page_text_dict_from_image_and_upload_image_to_blob(
        ...
        image_url = self.blob_storage_util.upload_file_to_blob_storage(
            type=self.constants.file_types.image,
    ...
    def get_file_analysis_json_for_zombie_file(self, file_path, pages_count):
        ...
        for i in range(1, pages_count):
            page_analysis_json = self.blob_storage_util.get_file_content_from_blob_storage(
                file_path=f"{file_path}/{self.constants.page_folder_prefix}{'{:03}'.format(i)}/{self.constants.file_names.page_analysis_json}"
            )
            page_analysis_json = json.loads(page_analysis_json)
            page_no = page_analysis_json[
                self.constants.page_analysis_parameters.page_number
            ]
            words_count_matched = page_analysis_json[
                self.constants.page_analysis_parameters.matched_keywords_count
            ]
            file_analysis_result[
                self.constants.file_analysis_parameters.pagewise_data
            ].append(
                {
                    self.constants.page_analysis_parameters.page_number: page_no,
                    self.constants.page_analysis_parameters.words_count_matched: words_count_matched,
                }
            )
            file_analysis_result[
                self.constants.file_analysis_parameters.words_count_matched
            ] += words_count_matched
        return file_analysis_result

    def initialize_file_analysis_result(self):
        return {
            self.constants.file_analysis_parameters.file_uri: None,
            self.constants.file_analysis_parameters.source: None,
            self.constants.file_analysis_parameters.number_of_pages: 0,
            self.constants.file_analysis_parameters.words_count_matched: 0,
            self.constants.file_analysis_parameters.duration_ms: 0,
            self.constants.file_analysis_parameters.pagewise_data: [],
        }

    def initialize_page_analysis_result(self):
        return {
            self.constants.page_analysis_parameters.page_number: None,
            self.constants.page_analysis_parameters.page_image: None,
            self.constants.page_analysis_parameters.duration_ms: 0,
            self.constants.page_analysis_parameters.matched_keywords_count: 0,
            self.constants.page_analysis_parameters.unique_matched_keywords_count: 0,
            self.constants.page_analysis_parameters.matched_keywords: [],
        }
    ...
    def handle_file_status_and_force_flag(
        ...
        if file_status == self.constants.statuses.processing:
            ...
            start_processing_from_page_no = self.get_no_of_pages_processed_for_file(
                f"{blob_storage_output_folder_path_file_level}/{self.constants.page_folder_prefix}"
            )
        ...
        elif file_status == self.constants.statuses.finished:
            process_file_with_already_extracted_text = True
        elif file_status == self.constants.statuses.failed:
            start_processing_from_page_no = self.get_no_of_pages_processed_for_file(
                f"{blob_storage_output_folder_path_file_level}/{self.constants.page_folder_prefix}"
            )
    ...
    def get_words_to_search(
        ...
        global_keywords_count = 0
        if search_scope == self.constants.search_scopes.local_scope:
            words_to_search = local_keywords
            local_keywords_count = len(local_keywords)
            global_keywords_count = 0
        elif search_scope == self.constants.search_scopes.global_scope:
            words_to_search = self.get_keywords_from_global_keywords(thread_id)
            local_keywords_count = 0
            global_keywords_count = len(words_to_search)
        elif search_scope == self.constants.search_scopes.both:
            local_keywords_count = len(local_keywords)
            global_keywords_count = len(
                self.get_keywords_from_global_keywords(thread_id)
            )
            words_to_search = local_keywords
            words_to_search.extend(self.get_keywords_from_global_keywords(thread_id))

        keywords_dict = {self.constants.keywords: words_to_search}
        keywords_json_url = self.blob_storage_util.upload_file_to_blob_storage(
            type=self.constants.file_types.json,
            file_data=keywords_dict,
            file_path=f"{blob_storage_output_folder_path_file_level}/{self.constants.file_names.keywords}",
        )
        return words_to_search, local_keywords_count, global_keywords_count
    ...
    def get_page_text_json(
        ...
        page_text_json = page_text_json[
            self.constants.vision_ai_api_response_parameters.read_result
        ]
        page_analysis[self.constants.page_analysis_parameters.page_number] = page_no
        page_analysis[self.constants.page_analysis_parameters.page_image] = image_url
    ...
    def concat_json_dict_data_into_string(self, json_dict):
        ...
        blocks = json_dict[self.constants.vision_ai_api_response_parameters.blocks]
        page_text_data = " ".join(
            line[self.constants.vision_ai_api_response_parameters.text]
            for block in blocks
            for line in block[self.constants.vision_ai_api_response_parameters.lines]
        )
    ...
    def pdf_doc_to_image(self, pdf_document, page_num):
        ...
        if (
            pix.width > self.constants.max_pix_width
            or pix.height > self.constants.max_pix_height
            or pix.width < self.constants.min_pix_width
            or pix.height < self.constants.min_pix_height
        ):
            raise FileNotSupportedException("Page dimensions are out of bounds.")
    ...
    def get_file_name_from_file_uri(self, file_uri, input_file_source):
        file_name = ""
        if input_file_source == self.constants.input_file_sources.blob:
    ...
    def process_page(
        ...
        blob_storage_output_folder_path_page_level = f"{blob_storage_output_folder_path_file_level}/{self.constants.page_folder_prefix}{'{:03}'.format(page_no)}"
        page_text_array_blob_path = f"{blob_storage_output_folder_path_page_level}/{self.constants.file_names.page_text_json}"
        page_image_blob_path = f"{blob_storage_output_folder_path_page_level}/{self.constants.file_names.page_image}"
        ...
        self.blob_storage_util.upload_file_to_blob_storage(
            type=self.constants.file_types.txt,
            file_data=page_text_data,
            file_path=f"{blob_storage_output_folder_path_page_level}/{self.constants.file_names.page_text}",
        )

        # Logging
        self.logger.info(
            f"Thread : {thread_id} :: {file_name} : Page No - {page_no} : Keywords analysis started with local {local_count} keywords and global {global_count} keywords"
        )
        keywords_analysis_start_time = time.time()
        ...
        keywords_analysis_complete_time = (
            time.time() - keywords_analysis_start_time
        ) * 1000
        duration_ms = (time.time() - page_processing_start_time) * 1000

        # Logging
        self.logger.info(
            f"Thread : {thread_id} :: {file_name} : Page No - {page_no} : Search keywords time: {keywords_analysis_complete_time}. Keywords analysis finished :: {duration_ms} milliseconds",
        )

        # Update page analysis and file analysis json with page processing details
        page_analysis[self.constants.page_analysis_parameters.duration_ms] = duration_ms
        page_analysis[
            self.constants.page_analysis_parameters.matched_keywords_count
        ] = page_analysis[
            self.constants.page_analysis_parameters.matched_keywords_count
        ...
        self.blob_storage_util.upload_file_to_blob_storage(
            ...
            file_path=f"{blob_storage_output_folder_path_page_level}/{self.constants.file_names.page_analysis_json}",
        )
    ...
    def process_queue_item(
        ...
        try:
            ...
            blob_storage_output_folder_path_file_level = (
                f"{self.constants.output_folder_name}/{base64_encoded_file_uri}"
            )
            ...
            self.logger.info(
                f"[Memory: {self.process_memory()}] Thread : {thread_id} :: {file_name} : {base64_encoded_file_uri} : Processing started",
            )
            ...
            self.update_progress_status(
                status=self.constants.statuses.processing,
            ...
            words_to_search, local_count, global_count = self.get_words_to_search(
                search_scope,
                local_keywords,
                blob_storage_output_folder_path_file_level,
                thread_id,
            ...
            if process_file_with_already_extracted_text:
                ...
                pdf_document, total_no_of_pages_in_file, queue_item = (
                    self.get_input_file_from_input_source(
                        ...
                        base64_encoded_file_uri=base64_encoded_file_uri,
                    )
                )
            self.logger.info(
                f"Thread : {thread_id} :: {file_name} : Total Pages: {total_no_of_pages_in_file}, Page analysis process started for {base64_encoded_file_uri}",
            )
            for i in range(
                ...
                if time.time() - start_Time > max_processing_time:
                    visibility_timeout = (time.time() - start_Time) + int(
                        self.global_constants.queue_visiblity_time
                    )
                    self.logger.info(
            ...
            file_analysis_result.update(
                {
                    self.constants.file_analysis_parameters.file_uri: file_uri,
                    self.constants.file_analysis_parameters.source: input_file_source,
                    self.constants.file_analysis_parameters.number_of_pages: total_no_of_pages_in_file,
                    self.constants.file_analysis_parameters.duration_ms: duration_ms,
                }
            )
            ...
            self.blob_storage_util.upload_file_to_blob_storage(
                ...
                file_path=f"{blob_storage_output_folder_path_file_level}/{self.constants.file_names.file_analysis_json}",
            )

            # Upload finished.lock file with count of pages to blob storage to indicate file processing finished
            self.update_progress_status(
                status=self.constants.statuses.finished,
                file_path=blob_storage_output_folder_path_file_level,
            ...
            self.update_progress_status(
                status=self.constants.statuses.failed,
                file_path=blob_storage_output_folder_path_file_level,
                content=self.constants.messages.unsupported_file_type,
                thread_id=thread_id,
            )
            ...
            self.update_progress_status(
                status=self.constants.statuses.failed,
                file_path=blob_storage_output_folder_path_file_level,
                content=self.constants.messages.conversion_error,
                thread_id=thread_id,
            )
        except ResourceNotFoundError as e:
            self.logger.exception(
            ...
            self.update_progress_status(
                status=self.constants.statuses.failed,
                file_path=blob_storage_output_folder_path_file_level,
                content=self.constants.messages.blob_not_found,
                thread_id=thread_id,
            )
        except Doc2PDFTimeoutError as e:
            self.logger.exception(
                f"Thread : {thread_id} :: {file_name} : Doc2PDFTimeoutError - {base64_encoded_file_uri} :: {str(e)}"
            )
            raise e
    ...
    def enqueue_api_request(
        ...
        if file_status == self.constants.statuses.processing:
            return {
                self.global_constants.api_response_parameters.status: self.global_constants.api_status_codes.bad_request,
                self.global_constants.api_response_parameters.message: self.constants.messages.file_is_in_processing,
            }

        # Add api payload to azure queue
        args = {
            self.constants.api_parameters.file_uri: file_uri,
            self.constants.api_parameters.source: input_file_source,
            self.constants.api_parameters.search_scope: search_scope,
            self.constants.api_parameters.keywords: local_keywords,
            self.constants.api_parameters.force: force,
        }

        # Upload queued.lock file to indicate file is queued for processing
        self.update_progress_status(
            status=self.constants.statuses.queued,
            file_path=blob_storage_output_folder_path_file_level,
        )
        ...
        return {
            self.global_constants.api_response_parameters.status: self.global_constants.api_status_codes.ok,
            self.global_constants.api_response_parameters.message: self.global_constants.api_response_messages.accepted,
            self.global_constants.api_response_parameters.identifier: base64_encoded_file_uri,
        }

    def get_status_of_file(self, file_url_base64_encoded):
        base_dir = f"{self.constants.output_folder_name}/{file_url_base64_encoded}"
        processing_lock_path = f"{base_dir}/{self.constants.file_names.processing_lock}"
        finished_lock_path = f"{base_dir}/{self.constants.file_names.finished_lock}"
        failed_lock_path = f"{base_dir}/{self.constants.file_names.failed_lock}"
        queued_lock_path = f"{base_dir}/{self.constants.file_names.queued_lock}"
        ...
        if self.blob_storage_util.check_if_directory_exists(base_dir):
            ...
            if self.blob_storage_util.check_if_directory_exists(
                ...
                return self.constants.statuses.processing, None
                ...
                return self.constants.statuses.success, {
                    self.constants.api_parameters.total_pages: int(no_of_pages),
                    self.constants.api_parameters.file_uri: f"{self.blob_storage_base_uri}/{base_dir}",
                }
                ...
                return self.constants.statuses.failed, {
                    self.global_constants.api_response_parameters.reason: f"{self.global_constants.api_response_messages.error_while_processing_file} : {error_message}"
                }

        return (
            self.constants.statuses.failed,
            self.constants.messages.invalid_file_identifier,
        )
